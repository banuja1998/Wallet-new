      ...
      {!isFetching && (
        <>
          ...
          <div className="utils page" style={{ marginTop: "-5px", marginBottom: "20px" }}>
            <div>
              <p style={{ margin: 0 }}>
                Organization: <strong>{currentUser?.organizationName || "Not assigned"}</strong>
              </p>
            </div>
...
function UsersTableHeader() {
  return (
    <tr>
      ...
      <th>Email</th>
      <th>Organization</th>
      <th>Tot. Expense(Rs.)</th>
      <th>Tot. Income(Rs.)</th>
      <th>Tot. No. Transactions</th>
      <th>Status</th>
      <th>Action</th>
    </tr>
  );
...
function UsersTableBody({ data, disableOrEnable }) {
  return data.map((item) => (
    <tr key={item.id}>
      ...
      <td>
        {item.enabled ? (
          <button onClick={() => disableOrEnable(item.id)} style={{ backgroundColor: "#ff0000" }}>
            Disable
          </button>
        ) : (
          <button onClick={() => disableOrEnable(item.id)} style={{ backgroundColor: "#6aa412" }}>
            Enable
          </button>
        )}
      </td>
